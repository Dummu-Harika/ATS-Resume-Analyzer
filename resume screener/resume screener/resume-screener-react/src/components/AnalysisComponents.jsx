...
export const AIInsightSection = ({ analysis }) => {
    ...
    return (
        <Card className="span-2 ai-insights">
            ...
            <div className="mt-4 pt-4 border-t" style={{ borderTop: '1px solid rgba(255,255,255,0.1)' }}>
...
export const FinalReportSection = ({ report }) => {
    ...
    return (
        <div className="report-container mt-8" style={{ width: '100%' }}>
            <h2 className="mb-6" style={{ borderBottom: '2px solid #6366f1', display: 'inline-block', paddingBottom: '8px' }}>
            ...
            <div className="grid grid-cols-2 gap-6 mb-6">
                <Card>
                    ...
                    <ul style={{ listStyle: 'disc', paddingLeft: '20px', fontSize: '0.9rem', color: '#cbd5e1' }}>
                        {report.experience_enhancement?.map((tip, i) => <li key={i} className="mb-2">{tip}</li>)}
                    </ul>
                </Card>
                <Card>
                    <h3 style={{ color: '#60a5fa', marginBottom: '15px' }}>Project Showcase Tips</h3>
                    <ul style={{ listStyle: 'disc', paddingLeft: '20px', fontSize: '0.9rem', color: '#cbd5e1' }}>
                        {report.project_showcase_tips?.map((tip, i) => <li key={i} className="mb-2">{tip}</li>)}
                    </ul>
                </Card>
            </div>
                ...
                <Card style={{ background: 'linear-gradient(135deg, rgba(99, 102, 241, 0.1) 0%, rgba(139, 92, 246, 0.1) 100%)', border: '1px solid rgba(99, 102, 241, 0.3)' }}>
                    ...
                    <div className="grid grid-cols-2 gap-8 mt-6">
                        {Object.entries(report.category_analysis || {}).map(([key, data]) => (
                            <div key={key} className="p-4 rounded-lg" style={{ background: 'rgba(0,0,0,0.2)' }}>
